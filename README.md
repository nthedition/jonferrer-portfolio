...
A1.Flex "out of host capacity" errors made recreating instances risky before
(see `lessons/phase-2b-oci-multi-node-cluster.md` §5). We only wipe and
reinstall the *software* (k3s), not the VMs.
...
(see prior conversation / `lessons/phase-2b-oci-multi-node-cluster.md` §2 for
the source). Prometheus's own scrape loop (every 30s by default) plus Grafana
